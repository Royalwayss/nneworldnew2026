...
class TagsController extends Controller
{
    ...
    {   
        ...
        if ($request->ajax()) { //
            ...
            return DataTables::of($query)
				...
				->addColumn('status', function ($user) use ($usersModule) {
                    ...
                    return '<a class="updateStatus" id="status-'.$user['id'].'" data-url="'.route('updateStatus',['tags',$user['id']]).'" data-id="'.$user['id'].'" style="color:'.$toggleColor.'" href="javascript:void(0)"> 
        ...
        return view('admin.tags.list', compact('usersModule'));
    }

    
    public function addeditTag(Request $request,$id='')
    {
			$property = [];
			$data['id'] = $id;
			if(!empty($id)){
				...
				$previous_id ='';
				if($id> 1){
					$get_previous_id = Tag::select('id')->where('id','<',$id)->where('is_delete','0')->orderby('id','desc')->first(); 
				...
				$next_id ='';
				$get_next_id = Tag::select('id')->where('id','>',$id)->where('is_delete','0')->orderby('id','asc')->first(); 
			...
			$html = (String)View::make('admin.tags.add-edit-form',$data);
